...
class Model(object):
	...
	def __init__(self, num_classes, seq_len, word_dict_size, wv, num_filters=100, emb_size=100, l2_reg_lambda = 0.01):
		...
		# model_path = './ckpt/baselines/lstm-att/model.ckpt' 
		...
		with tf.variable_scope('lstm'):
			...
			tmp1 = tf.matmul(tf.reshape(h1_rnn, shape=[-1, 2*num_filters]), W_a1, name="Wy") 	# NMx100
			h1_rnn = tf.reshape(tmp1, shape=[-1, seq_len, 2*num_filters])		 		#NxMx100

			M = tf.nn.relu(h1_rnn)									# NxMx100
			W_a2 = tf.get_variable("W_a2", shape=[2*num_filters, 1]) 				# 100 x 1
			tmp3 = tf.matmul(tf.reshape(M, shape=[-1, 2*num_filters]), W_a2)  		# NMx1
			alpha = tf.nn.softmax(tf.reshape(tmp3, shape=[-1, seq_len], name="att"))	# NxM	
			self.ret_alpha = alpha

			alpha = tf.expand_dims(alpha, 1) 						# Nx1xM
			h1_pool =  tf.matmul(alpha, h1_rnn, name="r")
