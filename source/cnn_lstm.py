...
class Model(object):
	...
	def __init__(self, num_classes, seq_len, word_dict_size, wv, num_filters=100, filter_size=4, emb_size=100, l2_reg_lambda = 0.01):

		tf.reset_default_graph()
		# model_path = './ckpt/baselines/cnn-lstm/model.ckpt' 
		...
		with tf.variable_scope('cnn'):
			...
			h1_pool=tf.nn.max_pool(h1_cnn, 
						ksize=[1,filter_size,1,1],
						strides=[1, 1, 1, 1],
						padding="VALID")
			h2_cnn = tf.squeeze(h1_pool, axis=[2])
		...
		with tf.variable_scope('lstm'):
			...
			# outputs = tf.stack(outputs, axis=1)
		 	...
		 	## Max pooling
			# h2_pool = tf.nn.max_pool(h1_rnn,ksize=[1, filter_size, 1, 1],strides=[1, 1, 1, 1], padding='VALID')
			h2_pool = tf.concat([outputs[0],outputs[-1]],axis=-1)
			
			
		
		##Dropout
		h_flat = tf.reshape(h2_pool,[-1,4*num_filters])
